...
import {
  ...
  getLocalAboutDraftMs,
  getProfileUpdatedAtMs,
  isAboutHeroColumnMissingError,
  omitAboutHeroFields,
  probeAboutHeroColumns,
  setAboutHeroColumnsAvailable,
...
export function ClassicAbout({ onBack, onHoverChange, isEditMode }: AboutProps) {
  ...
  const [bioParagraph1, setBioParagraph1] = useState(
    `I am a Principal Product Designer with a 20-year obsession with high-stakes, "zero-failure" systems. My focus lies at the intersection of complex hardware, regulated software, and frontier AI. I gravitate toward "gnarly" problems—situations where ambiguity is high, safety is critical, and the human impact is life-altering.`
  );
  ...
  const [aboutHeroHeadline, setAboutHeroHeadline] = useState(DEFAULT_ABOUT_HEADLINE);
  const [aboutHeroLead, setAboutHeroLead] = useState(DEFAULT_ABOUT_LEAD);
  ...
  const applyAboutPageFromStorageDraft = useCallback(
    (profileData: Record<string, unknown>, applyResume = true) => {
      ...
      if (profileData.about_hero_headline) setAboutHeroHeadline(profileData.about_hero_headline as string);
      if (profileData.about_hero_lead) setAboutHeroLead(profileData.about_hero_lead as string);
  ...
  const persistAboutPageDraftToLocalStorage = () => {
    const profileData = {
      about_hero_headline: aboutHeroHeadline,
      about_hero_lead: aboutHeroLead,
  ...
  }, [bioParagraph1, bioParagraph2, aboutHeroHeadline, aboutHeroLead, superPowersTitle, superPowers, highlightsTitle, highlights, leadershipTitle, leadershipItems]);
    ...
    const loadProfile = async () => {
      ...
      try {
        ...
        if (profile) {
          ...
          if (profile.about_hero_headline && profile.about_hero_headline.trim()) {
            setAboutHeroHeadline(profile.about_hero_headline);
          }

          if (profile.about_hero_lead && profile.about_hero_lead.trim()) {
            setAboutHeroLead(profile.about_hero_lead);
          }
  ...
  useEffect(() => {
    ...
    bioParagraph1, bioParagraph2, aboutHeroHeadline, aboutHeroLead, superPowersTitle, superPowers, 
  ...
  const saveToSupabase = async () => {
    ...
    try {
      ...
      if (isAboutHeroColumnMissingError(error)) {
        try {
          setAboutHeroColumnsAvailable(false);
          await updateCurrentUserProfile(omitAboutHeroFields(payload));
          console.log('✅ About page: Saved without hero columns (migration 0038 pending)');
          toast.warning(
            "Bio saved. Hero headline fields need a database update — run migration 0038 in the Supabase SQL editor (see supabase/migrations/0038_profiles_about_hero_fields.sql).",
            { duration: 14000 },
          );
          return;
        } catch (retryError) {
  ...
  const handleSave = (section: string) => {
    if (section === 'heroHeadline') {
      setAboutHeroHeadline(editedText);
    } else if (section === 'heroLead') {
      setAboutHeroLead(editedText);
    } else if (section === 'bio1') {
        ...
        >
          ...
          <div className="max-w-4xl relative z-10">
            ...
            <p className="text-xs uppercase tracking-widest text-muted-foreground mb-3">
              Hero (modern About — Section 1)
            </p>

            {isEditMode &&
              editingSection !== "heroHeadline" &&
              editingSection !== "heroLead" &&
              editingSection !== "bio1" &&
              editingSection !== "bio2" && (
              <div className="flex flex-wrap gap-2 mb-4">
                <Button
                  size="sm"
                  variant="outline"
                  onClick={() => handleEdit("heroHeadline", aboutHeroHeadline)}
                ...
                <Button
                  size="sm"
                  variant="outline"
                  onClick={() => handleEdit("heroLead", aboutHeroLead)}
                  className="rounded-full"
                >
                  <Edit2 className="w-3 h-3 mr-2" />
                  Edit Hero Lead
            ...
            {editingSection === "heroHeadline" ? (
              <div className="space-y-4 mb-6">
                ...
                <div className="flex gap-2 justify-end">
                  ...
                  <Button size="sm" onClick={() => handleSave("heroHeadline")}>
              ...
              <h2 className="text-2xl sm:text-3xl font-semibold leading-tight mb-4">{aboutHeroHeadline}</h2>
            )}

            {editingSection === "heroLead" ? (
              <div className="space-y-4 mb-8">
                <label className="block text-sm font-medium">Hero Lead</label>
                <Textarea
                  value={editedText}
                  onChange={(e) => setEditedText(e.target.value)}
                  className="min-h-[120px] text-lg"
                  placeholder="About page hero lead paragraph..."
                />
                <div className="flex gap-2 justify-end">
                  <Button size="sm" variant="outline" onClick={handleCancel}>
                    <X className="w-3 h-3 mr-1" />
                  ...
                  <Button size="sm" onClick={() => handleSave("heroLead")}>
                    <Save className="w-3 h-3 mr-1" />
                    Save
                  </Button>
                </div>
              ...
              <p className="text-lg leading-relaxed text-muted-foreground mb-8">{aboutHeroLead}</p>
            )}

            <p className="text-xs uppercase tracking-widest text-muted-foreground mb-3">
              Bio card (modern About — Section 2)
            </p>

            {/* Edit buttons for bio card paragraphs */}
            {isEditMode && editingSection !== 'heroHeadline' && editingSection !== 'heroLead' && editingSection !== 'bio1' && editingSection !== 'bio2' && (
            ...
            {editingSection === 'bio1' ? (
              <div className="space-y-4 mb-6">
                <label className="block text-sm font-medium">Bio Card — Paragraph 1</label>
                <Textarea
                  value={editedText}
                  onChange={(e) => setEditedText(e.target.value)}
                  className="min-h-[120px] text-lg"
            ...
            {editingSection === 'bio2' ? (
              <div className="space-y-4">
                <label className="block text-sm font-medium">Bio Card — Paragraph 2</label>
                <Textarea
                  value={editedText}
                  onChange={(e) => setEditedText(e.target.value)}
                  className="min-h-[120px] text-lg"
